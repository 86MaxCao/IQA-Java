...
/**
 * ONNX model manager for MANIQA (Multi-dimension Attention Network for IQA).
 *
 * <p>MANIQA uses a ViT backbone. The ONNX model takes a single 224x224 crop
 * and outputs a scalar quality score.
 */
public class MANIQAModelManager extends BaseONNXModelManager {

    private static final int CROP_SIZE = 224;

    public float runSingleCrop(float[] input) throws ModelException {
        if (!isLoaded()) {
            throw new ModelException("MANIQA model not loaded");
        }
        ...
        try {
            ...
            throw new ModelException("MANIQA inference failed: " + e.getMessage(), e);
        }
    }
