 ...
 *
 * <p>Uses 25 uniform 224x224 crops with ImageNet normalization.
 * The final score is the average across all crops.
 */
public class HyperIQAModel extends BaseIQAModel {

    private static final int CROP_SIZE = 224;
    private static final int NUM_CROPS = 25;

    private HyperIQAModelManager modelManager;
    private ImageDownloader imageDownloader;

    @Override
    protected void initializeModel(ModelConfig config) throws ModelException {
        modelManager = new HyperIQAModelManager();
        String modelPath = config.getModelPath("hyperiqa_model");
        if (modelPath == null) {
            modelPath = config.getModelPath("model");
        }
        if (modelPath == null) {
            throw new ModelException("Missing model path: 'hyperiqa_model' or 'model'");
        }
    ...
    protected double assessImageQuality(String imageUrl) throws ModelException {
        ...
        try {
            ...
            throw new ModelException("HyperIQA assessment failed: " + e.getMessage(), e);
        } finally {
