...
/**
 * ONNX model manager for TReS (Transformer for Relative Score).
 *
 * <p>TReS uses a ResNet50 backbone with a Transformer head. The ONNX model
 * is exported in eval mode (no flipped-image branch or consistency loss) and
 * takes a single 224x224 crop as input.
 */
public class TReSModelManager extends BaseONNXModelManager {

    private static final int CROP_SIZE = 224;

    public float runSingleCrop(float[] input) throws ModelException {
        if (!isLoaded()) {
            throw new ModelException("TReS model not loaded");
        }
        ...
        try {
            ...
            throw new ModelException("TReS inference failed: " + e.getMessage(), e);
        }
    }
